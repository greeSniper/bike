 ...
 * Created by 唐哲
 * 2018-06-08 15:16
 * 推送工具类：（用户扫单车上设备的二维码，扫到的就是设备的channelId，然后由度安卓端接收channelId发送给服务端）
 */
public class BaiduPushUtil {

    private BaiduPushUtil() {}

    private static BaiduPushUtil instance;

    public static BaiduPushUtil getInstance() {
        if(instance == null) {
            instance = new BaiduPushUtil();
        }
        return instance;
    }
